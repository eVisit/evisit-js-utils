	...
	async exec() {
		async function doOperation() {
			...
			try {
				...
				var newResult, events = ['beforeSuccess', 'success', 'complete', 'always'];
				...
				//Handle error result
				var newResult, events = ['beforeError', 'error', 'complete', 'always'];
				for (var i = 0, il = events.length; i < il; i++) {
					let eventName = events[i];
				...
				if (result)
					throw ((result instanceof Error) ? result : new Error(result));
			}
		}
		...
		if (this.cache && this.force !== true) {
			...
			if (!noe(cache) && cache.value) {
				if (cache.value instanceof Promise)
					return clone(await cache.value);
		...
		if (this.cache) {
			...
			result.then((result) => {
				return this.setCache(clone(result));
			}, (err) => {
