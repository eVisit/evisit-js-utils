...
function parseHTML(htmlStr) {
	...
	var uid = 1,
			currentNode = {name: 'body', type: 9, parent: null, children: [], id: uid++},
			docNode = currentNode;

	var parser = new htmlparser.Parser({
    onopentag: function(name, attributes) {
    	var node = currentNode = {type: 1, parent: currentNode, id: uid++};
      node.name = name;
    ...
    ontext: function(text) {
    	var node = {type: 3, parent: currentNode, id: uid++};
    	node.innerHTML = node.innerText = text;
    	currentNode.children.push(node);
    },
    onclosetag: function(tagname) {
    	currentNode.innerHTML = calculateInnerHTML(currentNode.children);
    	currentNode.innerText = calculateInnerHTML(currentNode.children, 3);
	...
	docNode.innerText = calculateInnerHTML(docNode.children, 3);
