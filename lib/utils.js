...
import htmlparser from '../plugins/htmlparser2/lib/index';
...
const defineROProperty = defineProperty.bind(this, false),
      defineRWProperty = defineProperty.bind(this, true);
...
function prop(cmd, _node, namespace) {
  ...
  if (op & GET) {
    ...
    if (!context || !(context instanceof Object) || context instanceof String || context instanceof Number || context instanceof Boolean)
...
/**
* @function {convertHTML} Parse an HTML string and run all nodes through a helper callback
* @param {String} {html} HTML string to parse
* @param {helper}
    @type {Function} Helper function. All DOM nodes (not real DOM nodes, just objects) will be given to this function
      @param {String} {html} HTML String to parse
      @param {Function} {helper} Callback function used to walk the HTML DOM tree (children are handled before parents)
    @end
  @end
* @return {*} Return whatever value the *helper* function returns
* @example {javascript}
  utils.convertHTML('<div style="color:red">This is a <b>test</></div>', function(node, children, parent) {
    //node === current node
    //children === The values of all children after being passed through the helper function FIRST
    //parent === parent node
  });
**/
...
/**
* @function {parseCSS} Parse inline CSS properties and return an object with key/value pairs. Property names with be camelCased
* @param {String} {cssStr} Inline style-sheet string
* @return {Object} Return an object of key/value pairs for the CSS style
* @example {javascript}
  utils.parseCSS('color:"red";font-size:"5pt";display:"inline";')
  // -> {color: "red", fontSize: "5pt", display: "inline"}
**/
...
function equal(obj) {
  function compare(obj1, obj2, alreadyVisited) {
    ...
    if (instanceOf(obj1, 'string') && instanceOf(obj2, 'string'))
    ...
    if (instanceOf(obj1, 'number') && instanceOf(obj2, 'number')) {
      ...
      if (!isFinite(obj1) && !isFinite(obj2))
        return true;
    ...
    if (instanceOf(obj1, 'boolean') && instanceOf(obj2, 'boolean'))
      return (obj1 == obj2); //Not triple equals because we are also testing Boolean instances
    ...
    if (instanceOf(obj1, 'array') && instanceOf(obj2, 'array')) {
      ...
      for (var i = 0, il = obj1.length; i < il; i++) {
        if (!compare(obj1[i], obj2[i], alreadyVisited))
          return false;
      }
    ...
    if (obj1.constructor !== Object.prototype.constructor || obj2.constructor !== Object.prototype.constructor)
      return (obj1.toString() === obj2.toString());
    ...
    if (!compare(keys1, keys2, alreadyVisited))
      return false;
    ...
    for (var i = 0, il = keys1.length; i < il; i++) {
      ...
      if (instanceOf(value1, 'string', 'number', 'boolean')) {
        ...
        if (alreadyVisited.indexOf(value1) >= 0)
          continue;

        alreadyVisited.push(value1);
        if (!compare(value1, value2, alreadyVisited))
          return false;
      }
    }
...
function parseURIQueryParams(str) {
  var params = {};

  (('' + str).split("?").slice(-1)[0]).replace(/\+/g, ' ').replace(/(&+)?([^=]+)=([^&]*)/g, function(m, junk, name, value) {
...
function formatterValidatorChainFactory(funcPool, _funcs, _args) {
          ...
          if (actualFunc instanceof Function) {
            (function(name, funcArgs, func) {
              finalFuncs.push(function(val, op, args) {
                var thisArgs = extend(true, {}, funcArgs, args),
                    ret;
  ...
  return function(val, op, userArgs) {
      ...
      if (op === 'validate' && ret) {
        if (instanceOf(ret, 'deferred')) {
          ret.then(function() {
            ...
            reject([...args, ...[val, op, funcArgs]]);
          });
        } else {
          reject(ret, [val, op, funcArgs]);
        }
    ...
    var formatterArgs = extend({}, userArgs, args);
    if (op === 'validate') {
...
root.equal = equal;
root.parseURIQueryParams = parseURIQueryParams;
